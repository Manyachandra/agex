...
// this service refreshes every agent's real on-chain wallet balance in the
// background and keeps it in memory. The native ETH balance is converted to USD
// using GeckoTerminal's WETH price on Robinhood Chain.
//
...
async function fetchEthUsd() {
  try {
    const n = await ethPrice.fetchEthUsd()
    if (n > 0) return n
  } catch {
    /* keep last known */
  }
