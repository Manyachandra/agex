// Real trading engine
//
// On an interval: fetch trending tokens, then for each eligible agent
// Eligible agents need a funded on-chain wallet; execute a REAL on-chain
// buy or sell of a trending token using the agent's own wallet. Records each
// trade to the DB and emits socket updates.
//
// SAFETY: this spends REAL money. Controls are read live from the `settings`
// table (id = 1) so they can be tuned without a restart. REAL_TRADE_* env vars
// are fallback defaults when a settings column is null.
// Exception: REAL_TRADING_ENABLED=false is a hard kill switch that always wins.
...
// House wallet that collects the per-trade fee (native ETH on Robinhood Chain).
...
async function loadConfig(supabase) {
  try {
    ...
    const s = data || {}
    // Env REAL_TRADING_ENABLED=false is a hard kill switch — overrides DB.
    // Env true / unset still allows the settings-table toggle to control live trading.
    const envHardOff = String(process.env.REAL_TRADING_ENABLED || '').toLowerCase() === 'false'
    lastConfig = {
      enabled: envHardOff
        ? false
        : (s.real_trading_enabled != null ? !!s.real_trading_enabled : DEFAULTS.enabled),
...
// Neutral commentary about a REAL on-chain trade the agent just made.
function buildTradeCommentary(_agent, side, symbol, ethAmount, tokenAmount) {
  const tk = `$${symbol}`
  ...
  const lines = {
    buy: [
      `Bought ${amt} ${tk} for ${eth} on Robinhood Chain.`,
      `Opened ${tk}: ${eth} for ${amt} tokens.`,
      `Added ${tk} with ${eth}.`,
    ],
    sell: [
      `Sold ${tk} for ${eth} on Robinhood Chain.`,
      `Closed ${tk} — ${eth} realized.`,
      `Exited ${tk} for ${eth}.`,
    ],
  }
  const pool = lines[side] || lines.buy
...
async function recordTrade(supabase, io, agent, side, token, result) {
  ...
  const activityRow = {
    agent_ticker: agent.ticker,
    action: `${emoji} REAL ${verb} ${tokenAmount.toLocaleString(undefined, { maximumFractionDigits: 4 })} $${result.symbol || token.symbol} for ${ethAmount.toFixed(6)} ETH on Robinhood Chain`,
    amount: ethAmount,
    action_type: 'real_trade',
    tx_hash: result.txHash || null,
  }
  // Store the tx hash so the Activity feed can link to Blockscout. If the
  ...
  try {
    const { data: rows } = await supabase.from('treasury').select('*')
    const treasury = (rows || []).sort((a, b) =>
      (parseFloat(b.total_fees) || 0) - (parseFloat(a.total_fees) || 0) ||
      (parseInt(b.total_trades, 10) || 0) - (parseInt(a.total_trades, 10) || 0)
    )[0]
    if (treasury) {
...
async function runCycle(supabase, io) {
  ...
  // Live ETH/USD (GeckoTerminal) so we can enforce the real-money minimum.
  let ethUsd = 0
  try {
    ethUsd = await ethPrice.fetchEthUsd()
  } catch { ethUsd = 0 }
...
// The scheduler always runs and re-reads config each tick, so settings can
// enable/disable trading and change the cadence live without a restart.
function start({ supabase, io }) {
  console.log('💱 Real trading engine scheduler started (controlled by settings table / env)')

  let stopped = false
  // Always re-check config on a short poll so toggles / env kill-switch apply
  // quickly. Actual trade cycles still respect cfg.intervalMs via lastRunAt.
  const POLL_MS = 15000
  ...
  const tick = async () => {
    ...
    if (!busy) {
      ...
      try {
        const cfg = await loadConfig(supabase)
        const due = !lastRunAt || (Date.now() - Date.parse(lastRunAt)) >= cfg.intervalMs
        if (cfg.enabled && due) {
