...
// V3 (or compatible router). Buy = ETH -> token, Sell = token -> ETH. Uses
// QuoterV2 for a price quote and applies slippage. Addresses are env-driven so
// Robinhood Chain (or another DEX) can be wired without another chain rewrite.
// Every swap spends real money, so all of this is gated by REAL_TRADING_ENABLED
// and per-trade caps in the scheduler.

const { ethers } = require('ethers')
const { getProvider } = require('./agentWallet')

const DEX_NOT_CONFIGURED =
  'DEX not configured for this chain. Set UNISWAP_V3_ROUTER, UNISWAP_V3_QUOTER, and WETH_ADDRESS.'

// ── Uniswap V3 / WETH — must be set via env for Robinhood Chain ─────────────
const WETH = process.env.WETH_ADDRESS || ''
const SWAP_ROUTER_02 = process.env.UNISWAP_V3_ROUTER || ''
const QUOTER_V2 = process.env.UNISWAP_V3_QUOTER || ''

function assertDexConfigured() {
  if (!WETH || !SWAP_ROUTER_02 || !QUOTER_V2) {
    const err = new Error(DEX_NOT_CONFIGURED)
    err.code = 'DEX_NOT_CONFIGURED'
    throw err
  }
}
...
// Public RPCs intermittently reject eth_call with "missing revert data" /
// CALL_EXCEPTION / network errors under load. These are transient, not real
// reverts, so retry read-only calls a few times with a short backoff.
...
/** Uniswap V3 swaps often need more gas than estimateGas (tick crossings). */
async function withGasBuffer(estimateFn, overrides = {}) {
  let gasLimit
  try {
    const est = await estimateFn()
    // 2x buffer — tight estimates on RH caused CALL_EXCEPTION with ~98% gas used.
    gasLimit = (est * 200n) / 100n
...
async function buyToken(signer, tokenAddress, ethAmount, slippage) {
  ...
  const overrides = await withGasBuffer(
    () => router.exactInputSingle.estimateGas(params, { value: amountIn }),
    { value: amountIn },
  )
...
async function sellToken(signer, tokenAddress, tokenAmount, slippage) {
  ...
  const overrides = await withGasBuffer(
    () => router.exactInputSingle.estimateGas(params),
  )
  const tx = await router.exactInputSingle(params, overrides)
  const receipt = await tx.wait()
