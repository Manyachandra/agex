  ...
  return (
    <div className="betting-slip">
        ...
        <div className="betting-amount-row">
          ...
          <div className="betting-quick-amounts">
            {[0.001, 0.005, 0.01, 0.05, 0.1].map(v => (
              <button key={v} onClick={() => setBetAmount(v.toString())} className="betting-quick-btn">{v}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      {betAmount && parseFloat(betAmount) >= 0.001 && (
        <div style={{ background: '#0a1628', border: '1px solid #1e2d3d', borderRadius: 8, padding: 12, marginBottom: 14 }}>
          <div style={{ fontSize: '0.73rem', color: '#94a3b8', marginBottom: 8, fontWeight: 600 }}>Example Payouts</div>
          {scenarios.map((s, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                <span style={{ color: s.right ? '#4ade80' : '#f87171' }}>{s.right ? '✅' : '❌'} {s.label}</span>
                <span style={{ fontWeight: 700, color: s.right ? '#4ade80' : '#f87171' }}>→ {payout} ETH</span>
              </div>
            )
          })}
...
export default function Betting() {
  ...
  const betSlipProps = { selectedAgent, selectedBetType, betAmount, setBetAmount, placing, isSending, isConfirming, txStatus, handlePlaceBet }
  ...
  return (
    <div className="betting-page">
      <style>{`
        .bet-slip-mobile { display: none; }
        .bet-slip-desktop { display: block; }
        @media (max-width: 768px) {
          .bet-slip-mobile { display: block; margin-top: 12px; }
          .bet-slip-desktop { display: none; }
        }
      `}</style>
          ...
          {tab === 'bet' && (
            <ScrollReveal delay={150}>
              <div className="betting-layout">
                      ...
                      <div key={agent.ticker}>
                        ...
                        >
                          ...
                          {isSelected && selectedBetType && (
                            <div style={{ marginTop: 8, padding: '8px 10px', background: '#071219', borderRadius: 6, fontSize: '0.75rem' }}>
                        ...
                        {/* MOBILE: bet slip inline below this agent */}
                        {isSelected && selectedBetType && (
                          <div className="bet-slip-mobile">
                            <BetSlipContent {...betSlipProps} />
                          </div>
                        )}
                      </div>
                    )
                ...
                {/* DESKTOP: bet slip in sidebar */}
                {selectedAgent && selectedBetType && (
                  <div className="bet-slip-desktop">
                    <BetSlipContent {...betSlipProps} />
                  </div>
                )}
